...
public class RegistrateCompany extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final DatabaseHelper databaseHelper = new DatabaseHelper(RegistrateCompany.this);
            ...
            branch.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (allCompanies.getSelectedItemPosition() == 0)
                        Toast.makeText(RegistrateCompany.this, "בחר עסק", Toast.LENGTH_SHORT).show();
                    else {
                        companyName = allCompanies.getSelectedItem().toString();
                        final SharedPreferences sp = getSharedPreferences("key", 0);
                        final SharedPreferences.Editor editor = sp.edit();
                        Company company = databaseHelper.getCompany(companyName);
                        editor.putString("companyName", companyName).apply();
                        if(company.getNet()) {
                            Intent intent = new Intent(RegistrateCompany.this, RegistrateBranch.class);
                            startActivity(intent);
                        }
                        else {
                            final AlertDialog.Builder alert = new AlertDialog.Builder(RegistrateCompany.this);
                            alert.setMessage("העסק שבחרת אינו רשת" + "\n" + "יש לך אפשרות ליצור משתמש בתוך העסק או לפתוח עסק חדש בקלות");
                                ...
                                public void onClick(DialogInterface dialogInterface, int i) {
                                    ...
                                    .setNegativeButton("עובד חדש", new DialogInterface.OnClickListener() {
                                        ...
                                        public void onClick(DialogInterface dialogInterface, int i) {
                                            ...
                                            databaseHelper.close();
                                            editor.putString("companyName", companyName).apply();
                                            editor.putString("branch", gson.toJson(branchName[0])).apply();
                                            Intent intent = new Intent(RegistrateCompany.this, RegistrateEmployee.class);
                                            startActivity(intent);
                                        }
                                    });
        ...
        //setting the different type of professions that are in the company
        ...
        type.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> adapterView, View view, int position, long l) {
                ...
                if (position != 5) {
                    ...
                    Button button = new Button(RegistrateCompany.this);
                    addButton(button, text1, false);
                    button = new Button(RegistrateCompany.this);
    ...
    public void addButton(Button button, String text, boolean belongToUser) {
        for (int i = 0; i < myProfessions.getChildCount(); i++) {
            ...
            if (check.getText().toString().equals(text)){
                Toast.makeText(RegistrateCompany.this, "כבר קיים תפקיד כזה", Toast.LENGTH_SHORT).show();
                return;
            }
        }
    ...
    class CreateCompany implements Runnable{
        ...
        public void run() {
            ...
            Company company = new Company(companyName,n,professions,typeSelected, isNet);
            Gson gson = new Gson();
            SharedPreferences sp = getSharedPreferences("key", 0);
            SharedPreferences.Editor editor = sp.edit();
            editor.putString("companyName", gson.toJson(company)).apply();
            Toast.makeText(RegistrateCompany.this, "צעד ראשון בוצע בהצלחה", Toast.LENGTH_SHORT).show();
